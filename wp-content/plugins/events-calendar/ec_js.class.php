		...
		for($d = 1; $d <= $lastDay; $d++) {
			...
			foreach($this->db->getDaysEvents($sqldate) as $e) {
				...
				if($output != '') {
					...
					$the_js  .= '	ecd.jq("#events-calendar-'.$d.'Large").append(\'<span class="event-block" id="events-calendar-'.$d.'Large">' . addslashes($titlinked) . '</span>\');
						ecd.jq("#events-calendar-'.$d.'Large")
							.attr("title", "")
							.mouseover(function() {
...
//(function($) {
	ecd.jq(document).ready(function() {
		...
		ecd.jq('#events-calendar-list-<?php echo $e->id;?>');
	});
//})(jQuery);
//]]>
</script>
