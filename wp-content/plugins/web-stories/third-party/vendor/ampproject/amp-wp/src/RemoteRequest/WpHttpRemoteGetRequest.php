...
final class WpHttpRemoteGetRequest implements RemoteGetRequest
{
    ...
    public function get($url)
    {
        ...
        do {
            $args = ['method' => 'GET', 'timeout' => $this->timeout, 'sslverify' => $this->ssl_verify];
            $response = wp_remote_get($url, $args);
            if ($response instanceof WP_Error) {
                return new RemoteGetRequestResponse($response->get_error_message(), [], 500);
            }
            if (!isset($response['response']['code'])) {
                return new RemoteGetRequestResponse(isset($response['response']['message']) ? $response['response']['message'] : 'Unknown error', [], 500);
            }
