...
class AMP_Meta_Sanitizer extends AMP_Base_Sanitizer
{
    ...
    private function get_body_meta_tag_name_attribute_deny_pattern()
    {
        static $pattern = null;
        if (null === $pattern) {
            $tag_spec = \current(\array_filter(AMP_Allowed_Tags_Generated::get_allowed_tag('meta'), static function ($spec) {
    ...
    public function sanitize()
    {
        ...
        foreach ($meta_elements as $meta_element) {
            ...
            if ($meta_element->hasAttribute(Attribute::CONTENT)) {
                $meta_element->setAttribute(Attribute::CONTENT, \preg_replace('/\\s*=\\s*/', '=', $meta_element->getAttribute(Attribute::CONTENT)));
            }
            /**
             * Meta tag to process.
             *
    ...
    {
        return AMP_DOM_Utils::create_node($this->dom, Tag::META, [Attribute::CHARSET => Document\Encoding::AMP]);
    }
    /**
    ...
    {
        return AMP_DOM_Utils::create_node($this->dom, Tag::META, [Attribute::NAME => Attribute::VIEWPORT, Attribute::CONTENT => $viewport]);
    }
    /**
