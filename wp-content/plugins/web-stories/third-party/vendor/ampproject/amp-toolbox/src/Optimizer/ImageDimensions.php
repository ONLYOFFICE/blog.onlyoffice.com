...
use Google\Web_Stories_Dependencies\AmpProject\LengthUnit;
final class ImageDimensions
{
    ...
    public function getDimensionsFromParent()
    {
        ...
        while ($element->parentNode && ++$level < 3) {
            ...
            $width = $element->hasAttribute(Attribute::WIDTH) ? $element->getAttribute(Attribute::WIDTH) : -1;
            $height = $element->hasAttribute(Attribute::HEIGHT) ? $element->getAttribute(Attribute::HEIGHT) : -1;
            if (empty($width)) {
                $width = -1;
            }
            if (empty($height)) {
    ...
    public function isTiny($threshold = self::TINY_THRESHOLD)
    {
        ...
        if (\in_array($widthUnit, LengthUnit::RELATIVE_UNITS, \true) && \in_array($heightUnit, LengthUnit::RELATIVE_UNITS, \true)) {
            return \false;
        }
        // If only one of the units is relative, compare the other against the threshold.
        if (\in_array($widthUnit, LengthUnit::RELATIVE_UNITS, \true)) {
            return \is_numeric($height) && $height < $threshold;
        } elseif (\in_array($heightUnit, LengthUnit::RELATIVE_UNITS, \true)) {
        ...
        switch ($this->getLayout()) {
            ...
            default:
                return \is_numeric($width) && $width < $threshold || \is_numeric($height) && $height < $threshold;
        }
    }
    /**
    ...
    public function getNumericWidth()
    {
        ...
        $width = \trim(\str_replace($widthUnit, '', $width));
        if (!\is_numeric($width)) {
            return \false;
        }
        $intWidth = (int) $width;
    ...
    public function getNumericHeight()
    {
        ...
        $height = \trim(\str_replace($heightUnit, '', $height));
        if (!\is_numeric($height)) {
            return \false;
        }
        $intHeight = (int) $height;
    ...
    public function getWidthUnit()
    {
        ...
        $this->widthUnit = \strtolower(\trim($matches['unit']));
        return $this->widthUnit;
    }
    /**
    ...
    public function getHeightUnit()
    {
        ...
        $this->heightUnit = \strtolower(\trim($matches['unit']));
        return $this->heightUnit;
    }
    /**
    ...
    {
        if ($this->layout === null) {
            $this->layout = $this->image->hasAttribute(Attribute::LAYOUT) ? (string) $this->image->getAttribute(Attribute::LAYOUT) : '';
        }
        return $this->layout;
    }
}
