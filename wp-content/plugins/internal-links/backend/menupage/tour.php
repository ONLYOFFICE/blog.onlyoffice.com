...
class Tour extends AbstractMenuPage
{
    ...
    public function __construct()
    {
        $this->steps = [
            ...
            [
                'slug' => 'links',
                'cta'  => __('Adjust the link behavior', 'ILJ'),
                'src'  => '\ILJ\Backend\MenuPage\Tour\Links'
            ],
            [
                'slug' => 'settings',
                'cta'  => __('Discover the most important settings', 'ILJ'),
                'src'  => '\ILJ\Backend\MenuPage\Tour\Settings'
            ],
            [
                'slug' => 'pro',
                'cta'  => __('Advanced linking with pro version', 'ILJ'),
        ...
        $this->page_title = __('Interactive Tour', 'ILJ');
    }
    ...
    protected function renderHeader()
    {
        ...
        echo '<p class="subline">' . __('Interactive tutorial', 'ILJ') . '</p>';
    ...
    protected function renderFooter()
    {
        ...
        $next_label     = (isset($next_step['cta'])) ? $next_step['cta'] : ((null === $previous_step) ? __('Start the tutorial now', 'ILJ') : ((null === $next_step) ? __('Finish tutorial', 'ILJ') : __('Next page', 'ILJ')));
        $previous_label = $this->action == 'after-activation' ? __('Skip interactive tour', 'ILJ') : __('Back to dashboard', 'ILJ');
        ...
        echo '</main>';
        echo '<footer>';
        echo '<div class="left">' . ($previous_step ? sprintf('<a href="%1$s" class="button">&lsaquo; ' . __('Previous page', 'ILJ') . '</a>', $previous_step['url']) : '') . '</div><div class="' . ($previous_step ? 'right' : ' only') . '">' . sprintf('<a href="%1$s" class="button button-primary">%2$s &rsaquo;</a>', $next_step['url'], $next_label) . '</div><div class="clear"></div>';
        echo '</footer>';
        echo '</div>';
