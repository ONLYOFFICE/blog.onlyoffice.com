...
class Settings extends AbstractMenuPage
{
    ...
    public function __construct()
    {
        ...
        $this->page_title = __( 'Settings', 'ILJ' );
        $this->tabs = [ [
            'slug'     => Options::ILJ_OPTION_SECTION_GENERAL,
            'title'    => __( 'General', 'ILJ' ),
            'callback' => function () {
    ...
    public function register()
    {
        ...
        wp_register_script(
            'ilj_select2',
            ILJ_URL . 'admin/js/select2.js',
            [],
            ILJ_VERSION
        );
        wp_localize_script( 'ilj_select2', 'ilj_select2_translation', [
            "error_loading"   => __( 'The results could not be loaded.', 'ILJ' ),
            "input_too_short" => __( 'Minimum characters to start search', 'ILJ' ),
            "loading_more"    => __( 'Loading more results…', 'ILJ' ),
            "no_results"      => __( 'No results found', 'ILJ' ),
            "searching"       => __( 'Searching…', 'ILJ' ),
        ] );
        $this->addAssets( [
            'tipso'                        => ILJ_URL . 'admin/js/tipso.js',
            'ilj_menu_' . $this->page_slug => ILJ_URL . 'admin/js/ilj_menu_settings.js',
            'ilj_select2'                  => ILJ_URL . 'admin/js/select2.js',
            'ilj_promo'                    => ILJ_URL . 'admin/js/ilj_promo.js',
        ], [
            'tipso'                        => ILJ_URL . 'admin/css/tipso.css',
            'ilj_menu_' . $this->page_slug => ILJ_URL . 'admin/css/ilj_menu_settings.css',
    ...
    public function render()
    {
        ...
        $this->renderHeadline( __( 'Settings', 'ILJ' ) );
        echo  '<div class="row">' ;
        ...
        if ( !$ilj_fs->is__premium_only() || !$ilj_fs->can_use_premium_code() ) {
            echo  '<a href="' . get_admin_url( null, 'admin.php?page=' . AdminMenu::ILJ_MENUPAGE_SLUG . '-pricing' ) . '" class="upgrade">' . __( 'Upgrade to Pro now - unlock all features', 'ILJ' ) . ' <span class="dashicons dashicons-unlock"></span></a>' ;
        }
        echo  '</h2>' ;
        ...
        echo  '<div>' ;
        echo  '<p class="submit">' ;
        echo  '<input type="submit" name="ilj-reset-options" class="button button-secondary" value="' . __( 'Reset Options', 'ILJ' ) . '" onclick="return confirm(\'' . __( 'You are going to overwrite the existing settings in this section with the defaults.', 'ILJ' ) . '\');">' ;
        echo  '</p>' ;
        echo  '</div>' ;
        echo  '</footer>' ;
        echo  '</section>' ;
